...
export const ProductView = ({ initialProduct }: ProductViewProps) => {
  const { product, handleDownload } = useProductDetail({ initialProduct });
  ...
  return (
    <ShopLayout
      ...
      pageDesc={ `${ product.name }: ${ product.desc }` }
      pageKeywords={ `${ product.name }, GNSS, RTK, PPK, NTRIP, Müpütun, Mettatec` }
      pageImage={ product.imgs[0]?.url ?? '' }
    >
        ...
        <section className='mx-auto grid max-w-7xl items-start gap-10 px-6 pb-20 pt-16 lg:grid-cols-[minmax(0,1.35fr)_minmax(380px,0.65fr)] lg:px-8 lg:pb-28 lg:pt-20'>
          ...
          <aside className='relative self-start rounded-[2.5rem] bg-white/50 p-6 shadow-[inset_0_1px_0_rgba(255,255,255,0.78),0_28px_90px_rgba(15,23,42,0.10)] backdrop-blur-2xl lg:sticky lg:top-28 lg:p-8'>
            ...
            <div className='mt-7 rounded-[1.5rem] bg-white/46 p-4 text-sm leading-6 text-slate-600 shadow-[inset_0_1px_0_rgba(255,255,255,0.72)] backdrop-blur-2xl'>
              Validamos stock, configuración, accesorios y compatibilidad antes de coordinar la entrega.
            </div>
        ...
        <section className='mx-auto max-w-7xl px-6 pb-24 lg:px-8'>
          <div className='mb-10 max-w-3xl'>
            ...
            <h2 className='mt-4 text-4xl font-semibold tracking-normal text-slate-950 sm:text-5xl'>
              Datos técnicos para elegir con criterio.
            </h2>
          </div>
