...
export const BlogCard = ({ blog, index = 0 }: BlogCardProps) => {
  ...
  return (
    <article className='group relative min-h-[520px] overflow-hidden rounded-[2.75rem] bg-white/10 shadow-[0_28px_92px_rgba(15,23,42,0.12)] transition duration-500 hover:-translate-y-1 hover:shadow-[0_38px_120px_rgba(8,145,178,0.18)] lg:min-h-[560px]'>
      <NextLink
      ...
      <div className={`absolute bottom-5 top-auto z-10 rounded-[2.15rem] bg-white/50 p-6 shadow-[inset_0_1px_0_rgba(255,255,255,0.74),0_24px_78px_rgba(15,23,42,0.18)] backdrop-blur-2xl transition duration-500 group-hover:bg-white/58 lg:bottom-8 lg:max-w-2xl lg:p-8 ${ isReversed ? 'left-5 right-5 lg:left-8 lg:right-auto' : 'left-5 right-5 lg:left-auto lg:right-8' }`}>
        <p className='mb-3 text-xs font-semibold uppercase tracking-[0.24em] text-cyan-700'>
          Lectura técnica
        </p>
        <h3 className='text-2xl font-semibold leading-tight text-slate-950 sm:text-3xl lg:text-4xl'>
          { blog.title }
        </h3>
        <p className='mt-4 line-clamp-4 text-sm leading-6 text-slate-600 sm:text-base sm:leading-7'>
          { blog.desc }
        </p>
