export const productsDB = [
  {
    id: 'p-001',
    name: 'X5 Linea para celulares',
    cate: 'gnss-x5',
    desc: 'El receptor de antena GNSS externo de alta precisión que funciona con tu smartphone o tableta Android o iOS. La solución perfecta para usuarios profesionales tanto principiantes como con experiencia en campo. Cuenta con un sistema de corrección NTRIP que recibe datos a través de Internet o Bluetooth. Tiene un indicador láser de punto rojo para facilitar el uso en levantamientos geodésicos.',
    banner: 'https://mettatec.com/wp-content/uploads/2023/09/PPKNTRIP-GNSS-X5Mobile-for-Smartphones-on-field-2.webp',
    techSpecs: [
      {
        primary: 'Utiliza la misma tecnología GNSS L1/L2/e5 encontrada en receptores profesionales, con componentes de vanguardia',
        secondary: ''
      },
      {
        primary: 'Recepción simultánea de todas las constelaciones: GPS, GLONASS, GALILEO y BEIDOU',
        secondary: ''
      },
      {
        primary: 'Correcciones NTRIP multibanda con tiempos de convergencia rápidos y rendimiento robusto, ofreciendo precisión a nivel de centímetros con una solución de posición fija en segundos',
        secondary: ''
      },
      {
        primary: 'Permite Levantamientos Geodésicos y Fotogrametría con georreferenciación de alta precisión',
        secondary: ''
      },
      {
        primary: 'Incluye su propia antena GNSS helicoidal multibanda',
        secondary: ''
      },
      {
        primary: 'Conexión para transferencia de datos a través de puerto USB y Bluetooth',
        secondary: ''
      },
      {
        primary: 'Compatible con aplicaciones de SIG y Fotogrametría que operan con datos NMEA',
        secondary: ''
      },
      {
        primary: 'Posee un láser rojo para facilitar la localización de puntos a georreferenciar en el campo',
        secondary: ''
      },
      {
        primary: 'Compatibilidad con dispositivos Android e iOS',
        secondary: ''
      },
      {
        primary: 'En resumen: Precisión a nivel de centímetros en un módulo pequeño y de muy bajo consumo energético',
        secondary: ''
      },
    ],
    imgs: [
      { 
        name: 'img-1',
        url: 'https://mettatec.com/wp-content/uploads/2023/09/PPKNTRIP-GNSS-X5Mobile-for-Smartphones.webp'
      },
      { 
        name: 'img-2',
        url: 'https://mettatec.com/wp-content/uploads/2023/09/PPKNTRIP-GNSS-X5Mobile-for-Smartphones-Components.webp'
      },
      { 
        name: 'img-3',
        url: 'https://mettatec.com/wp-content/uploads/2023/09/PPKNTRIP-GNSS-X5Mobile-for-Smartphones-LASER.webp'
      },
    ],
    price: '$0.000.000',
    slug: 'x5-mobile-series',
    doc: 'X5M.pdf'
  },
  {
    id: 'p-002',
    name: 'X5 Receptor GNSS Multibanda',
    cate: 'gnss-x5',
    desc: 'Estación base y Rover para soluciones fijas PPK, RTK y NTRIP. Con solo unos minutos de configuración, el X5R es uno de los métodos más rápidos para tomar mediciones de grado centimétrico. Funciona con software GIS común, incluyendo SW Maps, SurvPC, Field Genius y muchos otros.',
    banner: 'https://mettatec.com/wp-content/uploads/2023/09/X5R-OCT-2023-TIENDA-01.webp',
    techSpecs: [
      {
        primary: 'Módulo GNSS independiente',
        secondary: ''
      },
      {
        primary: 'Rover y Base de Posicionamiento GNSS para post-procesamiento',
        secondary: ''
      },
      {
        primary: 'Posicionamiento GNSS con RTK LoRa (precisión de 10 mm) con un alcance de 10 km en línea de vista',
        secondary: ''
      },
      {
        primary: 'Posicionamiento GNSS con NTRIP (precisión de 10 mm) recibe datos de corrección en streaming de un caster NTRIP para aplicar como correcciones en tiempo real a un receptor GNSS',
        secondary: ''
      },
    ],
    imgs: [
      { 
        name: 'img-1',
        url: 'https://mettatec.com/wp-content/uploads/2023/09/X5-Multi-band-GNSS-Receiver-PPK-RTK-NTRIP.webp'
      },
      { 
        name: 'img-2',
        url: 'https://mettatec.com/wp-content/uploads/2023/09/X5-Multi-band-GNSS-Receiver-PPK-RTK-NTRIP-POV-Down.webp'
      },
      { 
        name: 'img-3',
        url: 'https://mettatec.com/wp-content/uploads/2023/09/GNSS-Receiver-X5R-Plus-Compatible-with-the-best-RTK-Drones-of-the-Market.webp'
      },
    ],
    price: '$0.000.000',
    slug: 'x5-receptor',
    doc: 'X5R.pdf'
  },
  {
    id: 'p-003',
    name: '',
    cate: '',
    desc: '',
    banner: '',
    techSpecs: [
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
    ],
    imgs: [
      { 
        name: 'img-1',
        url: ''
      },
      { 
        name: 'img-2',
        url: ''
      },
      { 
        name: 'img-3',
        url: ''
      },
    ],
    price: '$0.000.000',
    slug: '',
    doc: ''
  },
  {
    id: 'p-004',
    name: '',
    cate: '',
    desc: '',
    banner: '',
    techSpecs: [
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
    ],
    imgs: [
      { 
        name: 'img-1',
        url: ''
      },
      { 
        name: 'img-2',
        url: ''
      },
      { 
        name: 'img-3',
        url: ''
      },
    ],
    price: '$0.000.000',
    slug: '',
    doc: ''
  },
  {
    id: 'p-005',
    name: '',
    cate: '',
    desc: '',
    banner: '',
    techSpecs: [
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
    ],
    imgs: [
      { 
        name: 'img-1',
        url: ''
      },
      { 
        name: 'img-2',
        url: ''
      },
      { 
        name: 'img-3',
        url: ''
      },
    ],
    price: '$0.000.000',
    slug: '',
    doc: ''
  },
  {
    id: 'p-006',
    name: '',
    cate: '',
    desc: '',
    banner: '',
    techSpecs: [
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
    ],
    imgs: [
      { 
        name: 'img-1',
        url: ''
      },
      { 
        name: 'img-2',
        url: ''
      },
      { 
        name: 'img-3',
        url: ''
      },
    ],
    price: '$0.000.000',
    slug: '',
    doc: ''
  },
  {
    id: 'p-007',
    name: '',
    cate: '',
    desc: '',
    banner: '',
    techSpecs: [
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
    ],
    imgs: [
      { 
        name: 'img-1',
        url: ''
      },
      { 
        name: 'img-2',
        url: ''
      },
      { 
        name: 'img-3',
        url: ''
      },
    ],
    price: '$0.000.000',
    slug: '',
    doc: ''
  },
  {
    id: 'p-008',
    name: '',
    cate: '',
    desc: '',
    banner: '',
    techSpecs: [
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
    ],
    imgs: [
      { 
        name: 'img-1',
        url: ''
      },
      { 
        name: 'img-2',
        url: ''
      },
      { 
        name: 'img-3',
        url: ''
      },
    ],
    price: '$0.000.000',
    slug: '',
    doc: ''
  },
  {
    id: 'p-009',
    name: '',
    cate: '',
    desc: '',
    banner: '',
    techSpecs: [
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
    ],
    imgs: [
      { 
        name: 'img-1',
        url: ''
      },
      { 
        name: 'img-2',
        url: ''
      },
      { 
        name: 'img-3',
        url: ''
      },
    ],
    price: '$0.000.000',
    slug: '',
    doc: ''
  },
  {
    id: 'p-010',
    name: '',
    cate: '',
    desc: '',
    banner: '',
    techSpecs: [
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
    ],
    imgs: [
      { 
        name: 'img-1',
        url: ''
      },
      { 
        name: 'img-2',
        url: ''
      },
      { 
        name: 'img-3',
        url: ''
      },
    ],
    price: '$0.000.000',
    slug: '',
    doc: ''
  },
  {
    id: 'p-011',
    name: '',
    cate: '',
    desc: '',
    banner: '',
    techSpecs: [
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
    ],
    imgs: [
      { 
        name: 'img-1',
        url: ''
      },
      { 
        name: 'img-2',
        url: ''
      },
      { 
        name: 'img-3',
        url: ''
      },
    ],
    price: '$0.000.000',
    slug: '',
    doc: ''
  },
  {
    id: 'p-012',
    name: '',
    cate: '',
    desc: '',
    banner: '',
    techSpecs: [
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
      {
        primary: '',
        secondary: ''
      },
    ],
    imgs: [
      { 
        name: 'img-1',
        url: ''
      },
      { 
        name: 'img-2',
        url: ''
      },
      { 
        name: 'img-3',
        url: ''
      },
    ],
    price: '$0.000.000',
    slug: '',
    doc: ''
  },
];
